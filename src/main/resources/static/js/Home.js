...
function togglePlayPause() {
    ...
    if (audio.paused) {
        audio.play();
        // 如果你想在播放时改变播放按钮的图标，可以添加相应的代码
        // 例如，如果你使用Font Awesome，可以这样做：
        var playPauseButton = document.querySelector('.play-pause i');
...
function updateVolumeDisplay(volume) {
    // 如果没有传入volume，则使用当前音量
    volume = volume || document.getElementById('audio').volume;
    var volumeLevel = document.querySelector('.volume-level');
    volumeLevel.textContent = (volume * 100).toFixed(0) + '%';
}

// 监听音量滑块的input事件，以实时更新音量
...
//     setInterval(nextSlide, 2000); // 每隔3秒切换一次轮播图
// }
//
// startSlideshow();
